...
class BVHTree:
    ...
    def split_node(objects):
        ...
        return sorted_objects[min_axis][:min_idx + 1], sorted_objects[min_axis][min_idx + 1:], aabbs[axis][min_idx][0], aabbs[axis][min_idx][1] 

    @staticmethod
    def print_tree(nodes, leaves, i=0, depth=0):
        if nodes[i].data >= 0:
            print('\t' * depth, 'AABB: ', nodes[i].aabb, '  OBJS: ', leaves[nodes[i].data])
        print('\t' * depth, 'AABB: ', nodes[i].aabb)
        print(nodes, leaves, nodes[i].left, depth + 1)
        print(nodes, leaves, nodes[i].right, depth + 1)

    def build(self, objects, max_depth=8, max_leave_objects=1):
        ...
        self.tree_leaves.append(objects[1:])
