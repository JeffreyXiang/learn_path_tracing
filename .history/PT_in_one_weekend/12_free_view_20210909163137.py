...
class Camera:
    ...
    def get_rays_fast(self, rays: ti.template()):
        ...
        for i, j, k in ti.ndrange(width, height, rays.shape[2]):
            target = self.front_axis + (i / width - 0.5) * view_width * self.right_axis + (j / height - 0.5) * view_height * self.up_axis
    ...
    def get_rays(self, rays: ti.template()):
        ...
        for i, j, k in ti.ndrange(width, height, rays.shape[2]):
            target = self.focal_length[None] * (self.front_axis + ((i + ti.random(ti.f32)) / width - 0.5) * view_width * self.right_axis + ((j + ti.random(ti.f32)) / height - 0.5) * view_height * self.up_axis)
            sample = sample_in_disk()
            origin = self.aperture[None] / 2.0 * (sample[0] * self.right_axis + sample[1] * self.up_axis)
