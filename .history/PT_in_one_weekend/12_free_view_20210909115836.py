...
window = ti.ui.Window('Ray Tracer', resolution)
...
while gui.running:
    t = time.time() - t_base
    dt = t - t_last
    if gui.get_event((ti.GUI.PRESS, 'a')):
        left = True
    if gui.get_event((ti.GUI.RELEASE, 'a')):
        left = False
    if gui.get_event((ti.GUI.PRESS, 'd')):
        right = True
    if gui.get_event((ti.GUI.RELEASE, 'd')):
        right = False
    if left:
        theta -= dt
    if right:
        theta += dt
    camera.set_position(Vec3f([15 * np.cos(theta + epsilon), 2, 15 * np.sin(theta + epsilon)]))
