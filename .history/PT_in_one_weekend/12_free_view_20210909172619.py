...
Camera_info = ti.types.struct(resolution=Vec2i, fov=ti.f32, focal_length=ti.f32, aperture=ti.f32, position=Vec3f, front_axis=Vec3f, right_axis=Vec3f, up_axis=Vec3f)
...
class Camera:
    def __init__(self, resolution, fov=60, focal_length=1, aperture=0):
        self.info = Camera_info.field(shape=())
    ...
    def set_position(self, position):
        self.info[None].position = position

    def set_fov(self, fov):
        self.info[None].fov = float(fov)

    def set_len(self, focal_length=1, aperture=0):
        self.info[None].focal_length = float(focal_length)
        self.info[None].aperture = float(aperture)
    ...
    def look_at(self, target, roll=0):
        dir = (target - self.info[None].position).normalized()
        self.yaw = ti.atan2(-dir[0], -dir[2])
        self.pitch = ti.asin(dir[1])
        self.roll = float(roll) * np.pi / 180

    def update_attitude(self):
        self.info[None].position = self.position
        trans = rotate(self.yaw, self.pitch, self.roll)
        self.info[None].front_axis = trans @ Vec3f([0.0, 0.0, -1.0])
        self.info[None].right_axis = trans @ Vec3f([1.0, 0.0, 0.0])
        self.info[None].up_axis = trans @ Vec3f([0.0, 1.0, 0.0])

    def update_len(self):
        self.info[None].resolution = self.resolution
        self.info[None].fov = self.fov
        self.info[None].focal_length = self.focal_length
        self.info[None].aperture = self.aperture
    ...
    def move_front(self, d):
        self.position += d * self.front_axis

    def move_right(self, d):
        self.position += d * self.right_axis
    ...
    def get_rays_fast(self, rays: ti.template()):
        width = self.info[None].resolution[0]
        height = self.info[None].resolution[1]

        ratio = height / width
        view_width = 2 * ti.tan(self.info[None].fov * np.pi / 180)
        view_height = view_width * ratio
        
        for i, j, k in ti.ndrange(width, height, rays.shape[2]):
            target = self.info[None].front_axis + (i / width - 0.5) * view_width * self.info[None].right_axis + (j / height - 0.5) * view_height * self.info[None].up_axis
            rays[i, j, k].ro = self.info[None].position
    ...
    def get_rays(self, rays: ti.template()):
        ...
        for i, j, k in ti.ndrange(width, height, rays.shape[2]):
            target = self.info[None].focal_length * (self.info[None].front_axis + ((i + ti.random(ti.f32)) / width - 0.5) * view_width * self.info[None].right_axis + ((j + ti.random(ti.f32)) / height - 0.5) * view_height * self.info[None].up_axis)
            sample = sample_in_disk()
            origin = self.info[None].aperture / 2.0 * (sample[0] * self.info[None].right_axis + sample[1] * self.info[None].up_axis)
            rays[i, j, k].ro = self.info[None].position + origin
