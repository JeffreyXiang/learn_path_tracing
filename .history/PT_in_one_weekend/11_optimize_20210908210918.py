...
class BVHTree:
    ...
    def hit(self, ray):
        ...
        while stack_p >= 0:
            if self.tree_nodes_field[array_get(stack, stack_p,)].data >= 0:   
                record = object_list_hit(self.tree_leaves_field, self.tree_leaves_field_cut[array_get(stack, stack_p,)], self.tree_leaves_field_cut[array_get(stack, stack_p,) + 1], ray)
                if record.t > epsilon and (res.t < 0 or record.t < res.t): res = record
                stack_p -= 1
            elif self.tree_nodes_field[self.tree_nodes_field[array_get(stack, stack_p,)].left].data == -1:
                stack_p += 1
                array_set(stack, stack_p, self.tree_nodes_field[array_get(stack, stack_p,)].left)
            elif self.tree_nodes_field[self.tree_nodes_field[array_get(stack, stack_p,)].right].data == -1:
                stack_p += 1
                array_set(stack, stack_p, self.tree_nodes_field[array_get(stack, stack_p,)].right)
            else:
                self.tree_nodes_field[array_get(stack, stack_p,)].data = -2
                stack_p -= 1
        return res
