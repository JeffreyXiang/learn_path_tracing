...
class BVHTree:
    ...
    def build(self, objects, max_depth=8):
        ...
        self.tree_nodes.append(BVHNode(left=1, right=2, aabb=AABB(low=low, high=high), data=0))
    ...
    def hit(self, ray):
        ...
        while stack_p >= 0:
            ...
            if self.tree_nodes_field[cur_node].data >= -1:   
                if aabb_hit(self.tree_nodes_field[cur_node].aabb, ray):
                    if self.tree_nodes_field[cur_node].data >= 0:   
                        record = object_list_hit(self.tree_leaves_field, self.tree_leaves_field_cut[cur_node], self.tree_leaves_field_cut[cur_node + 1], ray)
                        if record.t > epsilon and (res.t < 0 or record.t < res.t): res = record
                        stack_p -= 1
                    else:
                        self.tree_nodes_field[cur_node].data = -2
                        stack_p += 1
                        array_set(stack, stack_p, self.tree_nodes_field[cur_node].left)
                else:
                    stack_p -= 1
            elif self.tree_nodes_field[cur_node].data == -2:
                self.tree_nodes_field[cur_node].data = -3
                stack_p += 1
                array_set(stack, stack_p, self.tree_nodes_field[cur_node].right)
            else:
                self.tree_nodes_field[cur_node].data = -1
                stack_p -= 1
        return res
