...
class BVHTree:
    ...
    def build(self, objects, max_depth=8):
        ...
        ti.root.dense(ti.i, self.tree_leaves_field_cut[len(self.tree_leaves)]).place(self.tree_leaves_field)
        for i in range(len(self.tree_leaves)):
    ...
    def hit(self, ray):
        ...
        while stack_p >= 0:
            if self.tree_nodes_field[stack[stack_p]].data >= 0:   
                record = sphere_hit(self.tree_leaves_field, self.tree_leaves_field_cut[stack[stack_p]], self.tree_leaves_field_cut[stack[stack_p] + 1], ray)
                if record.t > epsilon and (res.t < 0 or record.t < res.t): res = record
            elif self.tree_nodes_field[self.tree_nodes_field[stack[stack_p]].left].data <= -2:
                stack_p += 1
