...
class BVHTree:
    ...
    def build(self, objects, max_depth=8):
        ...
        ti.root.dense(ti.i, max(self.min_field_len, len(self.tree_nodes))).place(self.tree_nodes_field)
        for i in range(len(self.tree_nodes)):
            self.tree_nodes_field[i] = self.tree_nodes[i]

        # ti.root.dense(ti.i, max(self.min_field_len, len(self.tree_leaves)) + 1).place(self.tree_leaves_field_cut)
        # for i in range(len(self.tree_leaves)):
        #     self.tree_leaves_field_cut[i + 1] = self.tree_leaves_field_cut[i] + len(self.tree_leaves[i])

        # ti.root.dense(ti.i, max(self.min_field_len, self.tree_leaves_field_cut[len(self.tree_leaves)])).place(self.tree_leaves_field)
        # for i in range(len(self.tree_leaves)):
    ...
    def hit(self, ray):
        res = HitRecord(0.0)
        res.t = -1
        stack = ti.Vector(self.max_depth + 1, ti.i32)
        stack_p = 0
        stack[stack_p] = 0
        while stack_p >= 0:
