...
class BVHTree:
    ...
    @ti.func
    def hit(self, ray, stack_p):
        res = HitRecord(0.0)
        res.t = -1
        stack = ti.Vector.zero(ti.i32, self.max_depth + 1)
        stack[stack_p_by_value__] = 0
        while stack_p >= 0:
            if self.tree_nodes_field[stack[stack_p_by_value__]].data >= 0:   
                record = object_list_hit(self.tree_leaves_field, self.tree_leaves_field_cut[stack[stack_p_by_value__]], self.tree_leaves_field_cut[stack[stack_p_by_value__] + 1], ray)
                if record.t > epsilon and (res.t < 0 or record.t < res.t): res = record
                stack_p -= 1
            elif self.tree_nodes_field[self.tree_nodes_field[stack[stack_p_by_value__]].left].data == -1:
                stack_p += 1
                stack[stack_p_by_value__] = self.tree_nodes_field[stack[stack_p_by_value__]].left
            elif self.tree_nodes_field[self.tree_nodes_field[stack[stack_p_by_value__]].right].data == -1:
                stack_p += 1
                stack[stack_p_by_value__] = self.tree_nodes_field[stack[stack_p_by_value__]].right
            else:
                self.tree_nodes_field[stack[stack_p_by_value__]].data = -2
                stack_p -= 1
        return res
...
for i in range(50000):
    render()
    camera.set_position(Vec3f([15 * np.cos(0.1 * i), 2, 15 * np.sin(0.1 * i)]))
    gui.set_image(image)
    gui.show()
