...
BVHNode = ti.types.struct(left=ti.i32, right=ti.i32, low=Vec3f, high=Vec3f)
...
class World:
    def __init__(self, objects=[]):
        ...
        self.objects_num = 0
        self.objects_field = Sphere.field()
    ...
    def build(self):
        ...
        ti.root.dense(ti.i, self.objects_num).place(self.objects_field)
        for i in range(self.objects_num):
            self.objects_field[i] = self.objects[i]

    @ti.func
    def hit(self, ray):
        res = HitRecord(0.0)
        res.t = -1
        for i in ti.static(range(self.objects_num)):
            record = sphere_hit(self.objects[i], ray)
