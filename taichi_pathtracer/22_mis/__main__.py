...
spp = 1024
batch = 32
path_integrator_mis = NextEventEstimationPathIntegrator(
    propagate_limit=32,
    BSDF_importance_sampling=True,
    nee_multi_importance_sampling=True,
)
path_integrator_nmis = NextEventEstimationPathIntegrator(
    propagate_limit=32,
    BSDF_importance_sampling=True,
...
path_integrator_mis.prepare(world)
path_integrator_nmis.prepare(world)
path_integrator_rw.prepare(world)

# Render the final image
ti.sync()
start_time = time.time()
render(world, camera, path_integrator_mis)
ti.sync()
print(f"Time elapsed: {time.time() - start_time:.2f}s")

ti.tools.imwrite(image, 'outputs/22_mis_mis.png')

image.fill(0.0)
ti.sync()
start_time = time.time()
render(world, camera, path_integrator_nmis)
ti.sync()
print(f"Time elapsed: {time.time() - start_time:.2f}s")

ti.tools.imwrite(image, 'outputs/22_mis_nmis.png')
...
ti.tools.imwrite(image, 'outputs/22_mis_rw.png')
