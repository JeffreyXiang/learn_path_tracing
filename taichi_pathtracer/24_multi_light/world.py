...
from lights import Light, LightType, LightHitRecord
    ...
    def load_scene(self, scene):
        ...
        for mesh in scene.dump():
            if isinstance(mesh, trimesh.Trimesh):
                ...
                # If light source
                if visual.material.emissiveFactor is not None and np.sum(visual.material.emissiveFactor) > 0:
                    lights = [
                        Light(
                            type=LightType.TRIANGLE,
                            position=mesh.vertices[mesh.faces[i][0]],
                            x=mesh.vertices[mesh.faces[i][1]] - mesh.vertices[mesh.faces[i][0]],
                            y=mesh.vertices[mesh.faces[i][2]] - mesh.vertices[mesh.faces[i][0]],
                            radiance=visual.material.emissiveFactor,
                            material_id=mat_id
                        )
                    ...
                    triangles = [
                        Triangle(
                            ...
                            t0=visual.uv[mesh.faces[i][0]] if visual.uv is not None else (0.0, 0.0),
                            t1=visual.uv[mesh.faces[i][1]] if visual.uv is not None else (0.0, 0.0),
                            t2=visual.uv[mesh.faces[i][2]] if visual.uv is not None else (0.0, 0.0),
                            material_id=mat_id
                        ) for i in range(len(mesh.faces))
                    ]
