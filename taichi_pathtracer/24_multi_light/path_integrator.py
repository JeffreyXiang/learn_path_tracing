...
from light_bvh import LightBVH
...
class NextEventEstimationPathIntegrator(PathIntegrator):
    def __init__(
        ...
        self.use_light_bvh = use_light_bvh
        self.has_light = False
        self.lights_pdf = AliasTable(1)
        self.light_BVH = LightBVH(1)
    ...
    def prepare(self, world):
        ...
        if self.has_light:
            if not self.use_light_bvh:
                ...
                self.light_BVH = LightBVH(world.lights_BVH.primitive_cnt[None])
                self.light_BVH.build(world.lights_BVH.primitives, verbose=True)
    ...
    def run(self, ray: ti.template(), world: ti.template()):
        ...
        beta = Vec3f(1.0)
        pdf = 0.0
        last_hit_point = Vec3f(0.0)
        last_hit_normal = Vec3f(0.0)
        ...
        while True:
            ...
            # else, MIS if multiple importance sampling is enabled
                ...
                if not ti.math.isinf(pdf):  # inf pdf mean mirror reflection, so only use BSDF sampling
                    ...
                    if not self.use_light_bvh:
                        ...
                        prob = self.light_BVH.query(record.prim_id, last_hit_point, last_hit_normal)
            ...
            if not self.use_light_bvh:
                ...
                light_id, prob = self.light_BVH.sample(record.si.point, record.si.normal)
            ...
            last_hit_point = record.si.point
            last_hit_normal = record.si.normal
