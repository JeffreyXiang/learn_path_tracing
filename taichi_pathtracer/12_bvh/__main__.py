...
print(f'Average number of AABB intersection tests per ray: {image_nfe_aabb.mean()}')
plt.figure(figsize=(16, 8))
plt.axis("off")
plt.imshow(image_nfe_aabb)
plt.colorbar(shrink=0.5)
plt.tight_layout()
plt.savefig("outputs/12_bvh_nfe_aabb.png")
plt.close()

image_nfe_primitive = np.flip(image.to_numpy()[..., 1].T, axis=0)
print(f'Average number of primitive intersection tests per ray: {image_nfe_primitive.mean()}')
plt.figure(figsize=(16, 8))
plt.axis("off")
plt.imshow(image_nfe_primitive)
plt.colorbar(shrink=0.5)
plt.tight_layout()
plt.savefig("outputs/12_bvh_nfe_primitive.png")
plt.close()
...
ti.tools.imwrite(image, 'outputs/12_bvh.png')
