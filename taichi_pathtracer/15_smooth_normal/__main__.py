...
@ti.kernel
def shader_gn_vis(world: ti.template(), camera: ti.template()):
    for i, j, k in ti.ndrange(resolution[0], resolution[1], spp//batch):
        c = Vec3f(0.0)
        for b in range(batch):
            ray = camera.get_ray(i, j)
            hit, si, vis = world.hit(ray)
            c += (si.geo_normal * 0.5 + 0.5) / spp
        image[i, j] += c
...
# Visualize the smooth shading normal
image.fill(0)
shader_n_vis(world, camera)
ti.tools.imwrite(image, 'outputs/15_smooth_normal_n.png')

# Visualize the geometry normal
image.fill(0)
shader_gn_vis(world, camera)
ti.tools.imwrite(image, 'outputs/15_smooth_normal_geo_n.png')
...
ti.tools.imwrite(image, 'outputs/15_smooth_normal.png')
