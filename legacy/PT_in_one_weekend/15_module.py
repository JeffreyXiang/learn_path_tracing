...
texture_size = (2048 * 6, 2048)
texture_maxnum = 32
environment_size = (2048, 2048)
...
def load_environment(config):
    ...
    for record in config:
        ...
        if file_path[-3:] == 'exr':
            ...
            env = imageio.imread(file_path) / 255.0
...
class TextureManager:
    ...
    def add(self, file_path, id, size=None):
        if size is None:
            if os.path.exists(file_path):
                img = imageio.imread(file_path)
            else:
                img = imageio.imread(file_path + '_albedo.png')
            size = (img.shape[1], img.shape[0])
        self.configs.append({'file_path': file_path, 'size': size, 'id': id})

    def clear(self):
        self.configs = []

    def _traverse_tree(self, size):
        w, h = size
        for i in range(len(self.tree)):
            l, b, r, t = self.tree[i][0], self.tree[i][1], self.tree[i][2], self.tree[i][3]
            if r - l >= w and t - b >= h:
                self.tree[i] = [l, b + h, r, t]
                self.tree.insert(i, [l + w, b, r, b + h])
                return TextureArea(low=Vec2i([l, b]), high=Vec2i([l + w, b + h]))
        return None

    def build(self):
        self.tree = [[0, 0, self.size[0], self.size[1]]]
        self.configs.sort(key=lambda x: x['size'][0], reverse=True)
        self.configs.sort(key=lambda x: x['size'][1], reverse=True)
        for i in range(len(self.configs)):
            area = self._traverse_tree(self.configs[i]['size'])
            if area is None:
                raise MemoryError('Texture out of memory.')
            self.configs[i]['area'] = area
        
    def dump(self):
        return {'size': self.size, 'configs': self.configs}

    def load(self, data):
        self.size = data['size']
        self.configs = data['configs']
        for i in range(len(self.configs)):
            self.configs[i]['area'] = TextureArea(low=self.configs[i]['area']['low'], high=self.configs[i]['area']['high'])
...
@ti.data_oriented
class World:
    def __init__(self, texture_size=texture_size, environment_size=environment_size):
        self.spheres = []
        self.meshes = []
        self.environment = None
        self.spheres_bvh = None
        self.meshes_bvhs = []
        self.textures = TextureManager(texture_size)
        self.environments = TextureManager(environment_size)
    ...
    def build(self):
        self.textures.build()
        self.environments.build()
        load_texture(self.textures.configs)
        load_environment(self.environments.configs)
    ...
    def save(self, filename):
        data = {'meshes_bvhs': [], 'environment': self.environment, 'textures': self.textures.dump(), 'environments': self.environments.dump()}
    ...
    def load(self, filename):
        ...
        self.textures.load(data['textures'])
        self.environments.load(data['environments'])
        load_texture(self.textures.configs)
        load_environment(self.environments.configs)
...
def sphere_hit(object, ray):
    ...
    if discriminant >= 0:
        ...
        record.material.absorptivity = 0.25
...
world = World()
world.environments.add('./textures/cayley_interior_2k.exr', 0)
# positions = [Vec3f([100, 0, -100]), Vec3f([-100, 0, -100]), Vec3f([-100, 0, 100]), Vec3f([100, 0, 100])]
# normals = [Vec3f([0, 1, 0])]
# texture_coords = [Vec2i([0, 0]), Vec2i([10, 0]), Vec2i([10, 10]), Vec2i([0, 10])]
# indices = [
...
# ]
# world.textures.add('./textures/sandyground1', 0)
# world.add_mesh(positions, normals, texture_coords, indices)
positions, normals, texture_coords, indices, materials = load_obj('./models/Yoimiya/Yoimiya_ShapeChange.obj', 1, flip_z=True, flip_textcoord=True, transform=rotate(np.pi, 0))
for material in materials:
    world.textures.add(material['file_path'], material['id'])
world.add_mesh(positions, normals, texture_coords, indices)
...
for i in trange(32*8):
    render(moved=False)
    ti.imwrite(frame, '15_module.png')
